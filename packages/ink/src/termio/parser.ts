/** Streaming ANSI parser — semantic action generator. */
...
// Two width tables — emoji and East-Asian-wide — because we render into a
// monospace grid where any "wide" codepoint occupies two cells. These
// ranges intentionally use raw codepoint comparisons rather than calling
// out to a measurement library: the parser runs on every text token in
// the diff loop, and a 5-comparison branch beats any general-purpose
// table lookup. Ranges are conservative — they match what string-width
...
function parseCSI(rawSequence: string): Action | null {
  ...
  // Cursor save / restore (CSI variants — ESC 7/8 are handled by parseEsc).
  ...
  // DECSCUSR — set cursor style. Requires the space intermediate; without
  // it, the final byte `q` means DECSCA on some terminals, so we gate.
...
export class Parser {
  ...
  /** Feed input bytes and return the actions they decode to. */
  ...
  /** Emit a text action, splitting around any embedded BEL bytes. */
  ...
  private processSequence(seq: string): Action[] {
    ...
    switch (seqType) {
      case 'csi': {
        ...
        if (!action) return []
        // SGR is a style-state mutation, not a rendered action — it
        // changes the style applied to *subsequent* text actions.
        if (action.type === 'sgr') {
      ...
      case 'osc': {
        // Strip the introducer and the terminator. Either BEL or ESC \\
        // is valid; the tokenizer emits whichever the terminal sent.
      ...
      case 'ss3':
        // SS3 (ESC O ...) is application-mode cursor keys on input. When
        // we're parsing program output it's noise — surface as unknown
        // so callers that care can log, but don't try to interpret.
